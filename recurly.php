...
Plugin URI: http://www.logiccoding.com/recurly
...
function plans_shortcode($atts) {
	...
	if($plan_code != '') {
		...
		echo do_shortcode(get_plans($type));	
	}	
}
...
function get_plans($type) {
	...
	if(!empty($rs)){
		...
		foreach($plansArr as $plan) {
			...
			$shortcode .= "<div class='sub_div'>
				<a href='".get_page_link()."?plan_code=".$plan['plan_code']."'><strong>Choose</strong></a>
				</div>";
			$shortcode .= "<div style='clear:both;'></div>";
			$shortcode .= "</div>";
			$i++;	
...
function show_subscriptionform($plan_code) { 
	...
	if(!empty($rs)){
		...
		?>
			<link href="<?php echo site_url();?>/wp-content/plugins/recurly/recurlyjs/themes/default/recurly.css" rel="stylesheet">
		    <script src="<?php echo site_url();?>/wp-content/plugins/recurly/recurlyjs/js/recurly.js"></script>
		    <script>
